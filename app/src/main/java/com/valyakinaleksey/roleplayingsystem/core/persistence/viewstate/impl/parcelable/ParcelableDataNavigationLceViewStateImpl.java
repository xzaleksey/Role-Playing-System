...
public class ParcelableDataNavigationLceViewStateImpl<D extends Parcelable & RequestUpdateViewModel, V extends LceView<D>>
    extends AbsNavigationLceViewStateImpl<D, V, Parcelable> implements Parcelable {

  @Override public <S extends Parcelable> void addToPending(Action2<V, S> op, S data) {
    ParcelablePendingStateChangeWithDataImpl<V, S> dto =
        new ParcelablePendingStateChangeWithDataImpl<>();
    dto.setData(data);
  ...
  public static final Creator<ParcelableDataNavigationLceViewStateImpl> CREATOR =
      new Creator<ParcelableDataNavigationLceViewStateImpl>() {
        @Override public ParcelableDataNavigationLceViewStateImpl createFromParcel(Parcel in) {
          return new ParcelableDataNavigationLceViewStateImpl(in);
        }

        @Override public ParcelableDataNavigationLceViewStateImpl[] newArray(int size) {
  ...
  @Override public void writeToParcel(Parcel out, int flags) {
    ...
    for (int i = 0; i < size; i++) {
      ParcelablePendingStateChange<V> item =
          (ParcelablePendingStateChange<V>) pendingStateChangesList.get(i);
  ...
  private void writeLcePart(Parcel dest, int flags) {
    ...
    if (error != null) {
      ...
      dest.writeString(error.getBaseError().name());
      dest.writeString(error.getText());
    } else {
      dest.writeInt(0);
    }
    if (data != null) {
  ...
  private void readLcePart(Parcel in) {
    ...
    if (hasEnum == 1) {
      Class<BaseErrorType> errorType = (Class<BaseErrorType>) in.readSerializable();
      String err = in.readString();
      String errMessage = in.readString();
      this.error = new BaseError(Enum.valueOf(errorType, err), errMessage);
    }
