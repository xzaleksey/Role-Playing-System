...
public class AuthFragment extends AbsButterLceFragment<AuthComponent, AuthViewModel, AuthView>
  ...
  @BindView(R.id.email_input_layout) TextInputLayout emailInputLayout;
  ...
  @BindView(R.id.sign_up_button) Button signUpBtn;
  ...
  @BindString(R.string.error_min_symbols) String errorMinSymbols;

  public static AuthFragment newInstance() {
    return new AuthFragment();
  }

  @Override protected AuthComponent createComponent(String fragmentId) {
    return RpsApp.getAppComponent().getAuthComponent(new AuthModule(fragmentId));
  }
  ...
  @OnClick(R.id.sign_in_button) public void signIn() {
    action = () -> getComponent().getPresenter()
  ...
  @OnClick(R.id.forgot_password) public void resetPassword() {
    if (ValidationUtils.isValidEmail(etEmail.getText())) {
      MaterialDialog dialog =
          new MaterialDialog.Builder(getContext()).title(R.string.reset_password)
  ...
  @Override protected void initSubscriptions() {
    compositeSubscription.add(RxTextView.textChanges(etEmail).skip(1).subscribe(charSequence -> {
    ...
    compositeSubscription.add(RxTextView.textChanges(etPassword).skip(1).subscribe(charSequence -> {
  ...
  @NonNull private String updatePassword() {
    String s = etPassword.getText().toString();
    data.setPassword(s);
    return s;
  }

  @NonNull private String updateEmail() {
  ...
  private void startRequest() {
    if (TextUtils.isEmpty(emailInputLayout.getError()) && TextUtils.isEmpty(
        passwordInputLayout.getError())) {
      loadData();
    }
  }

  @OnClick(R.id.auth_button) public void googleAuth() {
