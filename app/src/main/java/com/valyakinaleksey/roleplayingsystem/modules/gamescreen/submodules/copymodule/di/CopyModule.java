...
@Module
public class CopyModule extends BaseFragmentModule {
    ...
    @Provides
    CopyViewState provideViewState(@Named(VIEW_STATE_FILE_NAME) ViewStateStorage storage) {
        return new CopyViewState(storage);
    }

    @Provides
    @GameScope
    CopyPresenter provideCommunicationBus(@Named(PRESENTER) CopyPresenter presenter, CopyViewState viewState) {
    ...
    @Named(PRESENTER)
    @GameScope
    CopyPresenter providePresenter() {
