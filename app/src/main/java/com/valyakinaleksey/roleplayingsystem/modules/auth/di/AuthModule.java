...
@Module public class AuthModule {
  ...
  @Provides @PerFragmentScope RegisterInteractor provideRegisterInteractor(
      FirebaseAuth firebaseAuth) {
    return new RegisterUseCase(firebaseAuth);
  }

  @Provides @PerFragmentScope ResetPasswordInteractor provideResetPasswordInteractor(
      FirebaseAuth firebaseAuth) {
  ...
  @Provides @Named(PRESENTER) @PerFragmentScope AuthPresenter provideAuthPresenter(
      LoginInteractor loginInteractor, RegisterInteractor registerInteractor,
    ...
    return new AuthPresenterImpl(loginInteractor, registerInteractor, resetPasswordInteractor,
        sharedPreferencesHelper, context, userGetInteractor);
  }

  @Provides @PerFragmentScope AuthPresenter provideCommunicationBus(
      @Named(PRESENTER) AuthPresenter presenter, AuthViewState viewState) {
